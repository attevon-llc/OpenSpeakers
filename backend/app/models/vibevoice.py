...
class VibeVoiceModel(TTSModelBase):
    ...
    def load(self, device: str = "cuda") -> None:
        ...
        # Pre-import torch._dynamo before vibevoice triggers it via transformers lazy imports.
        # Without this, torch._dynamo.package registers its CacheArtifact type twice
        # (once here, once via transformers.integrations.flex_attention) causing
        # "Artifact of type=precompile already registered in mega-cache artifact factory".
        import torch._dynamo  # noqa: F401

        if device == "cuda" and not torch.cuda.is_available():
            logger.warning("CUDA not available — falling back to CPU for VibeVoice 0.5B")
            device = "cpu"
    ...
    def stream_generate(self, request: GenerateRequest):
        """Yield PCM16 audio bytes as they are generated by the streaming model."""
        import copy
        import threading
        ...
        try:
            for audio_chunk in audio_streamer.get_stream(0):
                if torch.is_tensor(audio_chunk):
                    chunk_np = audio_chunk.detach().cpu().to(torch.float32).numpy()
                else:
                ...
                if peak > 1.0:
                    chunk_np = chunk_np / peak
                pcm16 = (np.clip(chunk_np, -1.0, 1.0) * 32767.0).astype(np.int16)
                yield pcm16.tobytes()
        finally:
            stop_event.set()
            audio_streamer.end()
